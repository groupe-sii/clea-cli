...
class Serve extends Bundler {
  ...
  getConfig () {
    ...
    let config = webpackMerge(super.getConfig(), webpackServe(this.project, this.options, swConfig));
  ...
  api () {
    ...
    let jsonServerConfigPath = path.resolve(this.project.root, this.options.apiCustomRoutes);
    ...
    if (fs.existsSync(jsonServerConfigPath)) {
      require(jsonServerConfigPath)(server);
    }
