...
class InitProject {
  ...
  start () {
    ...
    return this.copyStaticFiles()
      .then(() => (this.options.makeItProgressive) ? this.makeItProgressive() : Promise.resolve())
      .then(() => this.copyFiles({
        appName   : this.name,
      ...
      .then(() => {
        ...
        return (this.options.skipInstall) ? Promise.resolve() : this.installDependencies();
      })
  ...
  makeItProgressive () {
    return new Promise((resolve, reject) => {
      reject(`${chalk.blue('--make-it-progressive')} option is under active development. It will come as soon as possible.`);
    });
  }
