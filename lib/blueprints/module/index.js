...
class ModuleBlueprint extends Blueprints {
  ...
  beforeInstall () {
    return new Promise((resolve) => {
      if (this.options.component) {
        ...
        this.templateData.fileName = this.parsedPath.name;
      }
  ...
  afterInstall () {
    ...
    if (this.options.lazyLoad) {
      ...
      logger.pop(`  ${chalk.yellow('update')} ${DynamicPathParser.relative(this.project.root, pathToRouting)}`);
    }

    return defer
      .then(() => {
        if (this.options.lazyLoad) {
          logger.pop(`  ${chalk.green('INFO')} Module is lazy loaded in the ${DynamicPathParser.relative(this.project.root, pathToRouting)} routing file`);
        } else {
          logger.pop(`  ${chalk.yellow('WARNING')} Module is generated but not registered, it must be registered to be used`);
        }
