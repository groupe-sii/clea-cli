...
class Bundler {
  ...
  before () {
    return new Promise((resolve, reject) => {
      if (!this.project.isCleaProject()) {
        reject(`You have to be inside an project in order to use the ${chalk.green(this.type)} command.`);
      }

      if (this.project.clea.type === Project.TYPE.APPLICATION) {
        let rootPath = path.join(this.project.root, this.project.clea.root),
          sourceConfigPath = path.join(rootPath, this.project.clea.environments.source),
          configPath = path.join(rootPath, this.project.clea.environments[this.options.target]);

        if (this.project.clea.environments.source === undefined) {
        ...
        if (!fs.existsSync(sourceConfigPath)) {
          reject(`configuration file ${chalk.blue(this.project.clea.environments.source)} is missing.`);
        }
  ...
  getWebpackConfigFile (file) {
    switch (this.project.clea.type) {
      ...
      default:
        throw new Error(`No build configuration for ${this.project.clea.type} project type.`);
    }
  }

  checkWebpackConfigAvailable (list, file) {
    if (!list.includes(file)) {
      throw new Error(`No ${file} configuration for ${this.project.clea.type} project type.`);
    }
  }
